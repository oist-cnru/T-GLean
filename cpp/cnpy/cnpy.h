...
namespace cnpy {
    ...
    struct NpyArray {
        NpyArray(const std::vector<size_t>& _shape, char _dtype, size_t _word_size, bool _fortran_order) :
            shape(_shape), dtype(_dtype), word_size(_word_size), fortran_order(_fortran_order)
        {
            num_vals = 1;
        ...
        NpyArray() : shape(0), dtype(0), word_size(0), fortran_order(0), num_vals(0) { }
    ...
    void parse_npy_header(FILE* fp, char& dtype, size_t& word_size, std::vector<size_t>& shape, bool& fortran_order);
    void parse_npy_header(unsigned char* buffer, char& dtype, size_t& word_size, std::vector<size_t>& shape, bool& fortran_order);
    ...
    template<typename T> void npy_save(std::string fname, const T* data, const std::vector<size_t> shape, std::string mode = "w") {
        ...
        if(fp) {
            ...
            size_t word_size;
            bool fortran_order;
            parse_npy_header(fp,dtype,word_size,true_data_shape,fortran_order);
